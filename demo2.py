...
class DataPreprocessor:
    ...
    def load_and_preprocess_data(self):
        ...
        # 数据标准化
        features = ['Temp', 'Huminity', 'GHI', 'SHI','Rainfall', 'GTI', 'Power']
...
class Trainer:
    ...
    def evaluate(self, test_loader):
        ...
        # 计算总体正确率
        #threshold = 0.1  # 10%阈值
       # correct_predictions = np.abs(predictions - actuals) / actuals < threshold
        accuracy = 1-rmse

        return mae, mse, rmse,  accuracy, predictions, actuals
...
class ResultsSaver:
    ...
    def save_results(self, dates, predictions, actuals):
        # 确保 predictions 和 actuals 是一维数组
    ...
    def plot_results(self, dates, predictions, actuals, plot_filepath):
        ...
        plt.plot(dates, actuals, label='Actual Power', color='blue')
        plt.plot(dates, predictions, label='Predicted Power', color='red')
        plt.xlabel('Date')
        plt.ylabel('Power')
...
def main():
    # 文件路径
    data_filepath = 'Solibro.csv'  # 你的数据集路径
    result_filepath = 'prediction_results2.csv'
    plot_filepath = 'prediction_plot2.png'
    ...
    # 5. 模型评估
    mae, mse, rmse, accuracy, predictions, actuals = trainer.evaluate(test_loader)
    print(f"MAE: {mae:.8f}, MSE: {mse:.8f}, RMSE: {rmse:.8f}, Accuracy: {accuracy:.4%}")

    # 6. 保存结果并可视化
    results_saver = ResultsSaver(result_filepath)
    results_saver.save_results(dates[-len(actuals):], predictions, actuals)  # 只保存最后一部分日期
    results_saver.plot_results(dates[-len(actuals):], predictions, actuals, plot_filepath)
